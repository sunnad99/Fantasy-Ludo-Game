...
class Player:
    ...
    def initialize_player_vars(self):
        ...
        self.current_winning_path_tile_list = 0

        self.token_sprite_list = {} # For each respective color, we store a token list consisting of all the tokens present on the board
        self.token_movement_counter = {} # This keeps track of whether a token has returned back to its own tile set
        self.token_path_indice = {} # Keeps track of which path tile the tokens are currently on
    ...
    def show_current_player(self):
        self.settings.draw_text(f"Player {self.current_player}'s turn", self.settings.MAIN_MENU_FONT_PATH, 25, self.settings.BLACK, self.settings.screen_center, self.settings.box_size, "n")
...
# These are all the movement functions
    ...
    def move_on_normal_path(self):
        ...
        print("Destination_path_tile_indice:", indice_for_tile_to_move_to)
    ...
    def winning_path_tile_list_chooser(self):
        if self.current_player_color == "red":
            self.current_winning_path_tile_list = self.game.board.red_winning_path_sprites

        elif self.current_player_color == "green":
            self.current_winning_path_tile_list = self.game.board.green_winning_path_sprites

        elif self.current_player_color == "blue":
            self.current_winning_path_tile_list = self.game.board.blue_winning_path_sprites

        elif self.current_player_color == "yellow":
            self.current_winning_path_tile_list = self.game.board.yellow_winning_path_sprites
    ...
    def move_on_winning_path(self):
        if self.game.events.movement_checker > self.settings.winning_path_threshold: # This condition works if the dice value is greater than the amount needed to finish with token
            ...
            print("We will print a text saying you can't move")
            print("BUTTON SHOULD GO HERE")
            ...
            # Calls the method from the player module to choose the correct winning path sprite list
            self.winning_path_tile_list_chooser()
    ...
    def player_on_player_collision(self, indice_of_team_path_list):
        ...
        if safe_path_collision_condition:
            pass 
        else:
            for color in self.color_for_player:
                ...
                if color != self.current_player_color:
                    for current_token_indice, current_token in enumerate(self.token_sprite_list[color]):
                        total_step_taken_by_current_token = self.token_path_indice[color][current_token_indice]
                        current_token_path_indice = self.team_path[color][total_step_taken_by_current_token]

                        # The collision has taken place
                        if current_token_path_indice == indice_of_team_path_list:
                            ...
                            # Notify that a token has been taken back to base (with text on screen)
